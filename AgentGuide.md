...
1. 当用户提出“改变模型结构”“改变 hidden states / contextual pivot 的使用方式”“改变 noise/mask/block 构造”“改变 loss 或训练数据格式”时，修改完训练代码之后，必须同步检查并按需修改：
...
FlashMTP 的核心方向是去掉草稿模型侧 KV cache 对完整历史的依赖，利用目标大模型最新 hidden states / contextual pivot 表示历史上下文，并结合 block 内 mask/noise token 进行并行草稿预测。