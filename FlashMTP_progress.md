...
Streak-Distillation 的核心是：**不显式对齐逐点 KL**，而是近似最大化投机解码下的**期望接受长度**（长连续接受 streak），并用教师轨迹上的联合概率把「验证器按前缀条件接受」与「非自回归草案的并行预测」接起来。  
在本项目中，草案侧**不保留 KV cache**，条件输入只有目标模型给出的 **Contextual Pivot**（记为 $p$），而非整段 token 前缀；验证器 $P$ 仍按完整前文与已接受草案前缀做接受。  
因此与原文以序列前缀 $s$ 条件化 $Q_{\text{diff}}$ 的写法不同，需将草案条件统一改为 $p$；**详细定义、贪婪代理与目标函数见下文「v3」**。
...
**Step2 冲程蒸馏（Streak-Distillation）**；核心思想
...
| 记号 | 含义 |
|------|------|
| $p$ | **Contextual Pivot**：目标模型在「当前生成步」上对全历史的融合表征（可取多层 HS 融合），是草案 **唯一** 的显式条件，替代传统「草案输入 = 长前缀 token」的 $s$。 |
| $P$ | 验证模型（Teacher），给定真实前文与已实现上下文时的自回归分布；用于采样教师序列与评估接受。 |
| $Q_{\text{diff}}(\cdot \| p)$ | 离散扩散草案：在 **仅以 $p$ 为条件** 下并行预测块内 token（无草案侧 KV 展开前缀）。 |

验证阶段：接受概率 $\alpha_j$ 仍然依赖 **验证器眼里的前缀**（真实上下文 $\circ$ 已接受的 $x_{1:j-1}$），这是投机解码的机制；唯独 **草案生成分支** 不维护 token 级 cache，信息量由 $p$ 压缩注入。

---

**2. 核心原理（与原文对应，不改结论）**

1. **直接优化吞吐量**：最大化期望接受 token 数 $\text{Tokens}_{\text{Draft}}(\gamma, p)$，而非仅首 token 对齐。  
2. **贪婪接受代理**：用验证模型 $P$ 上的可微近似替代完整拒绝采样，使 streak 目标可训练。  
3. **沿教师轨迹优化**：在从 $P$ 采样的序列 $x_{1:\gamma}$ 上，抬升 $Q_{\text{diff}}$ 在这些 token 上的（因子化）联合质量，以鼓励 **长连续接受**。  
4. **桥接「联合并行预测」与「前缀条件接受」**：草案 $Q_{\text{diff}}(x_j \| p)$ 不显式依赖 $x_{1:j-1}$；通过期望与教师轨迹，把验证器的前缀依赖 **间接** 灌入训练目标（与原文同构，只是把 $s$ 换成草案条件 $p$）。
...
其中 $c$ 为与 pivot $p$ 对齐的验证侧前文；**要点**是 $\alpha_j$ 仍随已接受前缀变，而 $Q$ 侧条件始终是 $p$，不是逐 token 增长的草案 KV。

---

**4. 贪婪接受代理（草案项只用 $p$）**

第 $j$ 个位置的贪婪代理可写为由 $P$ 采样前缀、再评估草案在 $x_j$ 上的质量；草案分布使用 **Pivot 条件**：

（记与该 pivot 对齐的验证侧前文为 $c$）
...
其中 $Q_{\text{diff}}(x_j \,|\, p)$ 是扩散草案在仅给定 $p$ 下对位置 $j$ 的（边缘或与掩码时间表对应的）预测概率。教师侧前缀仍来自 $P$，与 FlashMTP「历史进 pivot、草案不展开了」一致。

---

**5. Streak-Distillation 目标（Pivot 版）**

在教师序列上最大化 streak  surrogate（条件为 $p$）：
...
- $(p,c)$ 从同一条训练样本联合得到：$c$ 为验证器可见前文，$p$ 为对应步上由目标大模型提取的 Contextual Pivot，二者对齐。  
- $x_{1:\gamma}$ 为自 $P(\cdot|c)$ 采样的教师轨迹。  
- $q_j(x_j \,|\, p\,;\theta)$ 为离散扩散草案在位置 $j$ 对 token $x_j$ 的概率；并行生成下联合近似为 $\prod_j q_j(\cdot \,|\, p)$。

该目标迫使草案在 **固定 pivot 信息量** 下，尽量复制教师在同一上下文下的 **长 streak**，从而对准投机解码的真实瓶颈，而非仅拟合首 token。

---

**小结**：v3 = MDLM 主训练（先不做强位置加权） + **Pivot 条件化的冲程蒸馏**；数学骨架仍是 Streak-Distillation，**唯一本质改写**是 $Q_{\text{diff}}$ 的条件从「前缀序列 $s$」变为「单 pivot $p$」，以匹配无 KV cache 设定。
