...
cd "$ROOT"
OUT="${RUN_ROOT:-$ROOT/benchmark_results/Flashmtp_v2_qwen3_8b_ep1_20260924/full}"
GPU="${GPU:-0}"
mkdir -p "$OUT"
exec 9>"$OUT/run.lock"
...
cp specforge/modeling/draft/flashmtp.py "$OUT/flashmtp.py"
printf 'running\n' > "$OUT/status"
trap 'printf "failed\n" > "$OUT/status"' ERR
trap 'printf "stopped\n" > "$OUT/status"; exit 143' TERM INT
for dataset in gsm8k math500 aime25 humaneval mbpp livecodebench mt-bench alpaca \
    ...
    "$PY" evaluation/benchmark.py \
        --model-name-or-path /data/wanghanzhen/models/Qwen3-8B \
        --draft-name-or-path "$ROOT/cache/models/Qwen3-8B/Flashmtp_v2_qwen3_8b_ep1" \
