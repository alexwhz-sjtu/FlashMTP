...
STAGE1_CE_WEIGHT="${STAGE1_CE_WEIGHT:-0.1}"
...
RUN_TAG="v23s_${DT}_${TARGET_TAG}_${DATA_TAG}_ws${WORLD_SIZE}_tp${TP_SIZE}_sh${SHARD_DRAFT_BY_TP}_tb${NAME_TARGET_BATCH}_db${NAME_DRAFT_BATCH}_${TEACHER_TAG}_i${STUDENT_INIT_MODE}${STUDENT_DEPTH_TAG}_m${MASK_TOKEN_ID}_e${STAGE1_EPOCHS}+t1+${STAGE2_EPOCHS}_lr${LEARNING_RATE}_w${WARMUP_RATIO}_kl${STAGE1_KL_WEIGHT}_h${STAGE1_HIDDEN_WEIGHT}_s1ce${STAGE1_CE_WEIGHT}_g${STAGE1_LOSS_DECAY_GAMMA:-none}_ce${STAGE2_FINAL_CE_WEIGHT}_tv${STAGE2_TV_WEIGHT}_b${STAGE2_BASE_CE_WEIGHT}_g${STAGE2_LOSS_DECAY_GAMMA:-none}_bg${STAGE2_BASE_CE_DECAY_GAMMA:-none}_L${MAX_LENGTH}_A${NUM_ANCHORS}_ac${ACCUMULATION_STEPS}"
...
WANDB_DEFAULT_BASE="v23s_${DT}_${TARGET_TAG}_${DATA_TAG}_ws${WORLD_SIZE}tp${TP_SIZE}sh${SHARD_DRAFT_BY_TP}_${TEACHER_WANDB_TAG}_e${STAGE1_EPOCHS}+t1+${STAGE2_EPOCHS}lr${LEARNING_RATE}_L${MAX_LENGTH}A${NUM_ANCHORS}"
...
CMD=(
  ...
  --stage1-ce-weight "${STAGE1_CE_WEIGHT}"
...
printf 'Transition: 1 epoch on Stage 2 data with cosine Stage1->Stage2 loss blending\n'
