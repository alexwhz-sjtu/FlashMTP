...
def parse_args():
    ...
    training_group.add_argument(
        ...
        type=str,
        default=None,
        help="Direct path to one checkpoint folder (model weights, config.json, "
        "training_state.pt, etc.). If set, this path is used as-is and automatic "
        "last-checkpoint discovery under --output-dir is skipped.",
    )
...
def record_metrics(
    ...
    mode: str = "train",
    prefix_acc: Optional[float] = None,
) -> None:
    logdict = {}
    ...
    extra = f", PrefixAcc: {prefix_acc:.4f}" if prefix_acc is not None else ""
    print_on_rank0(
        ...
        for step_in_epoch, data in enumerate(progress_bar):
            ...
            if global_step % args.log_interval == 0:
                ...
                record_metrics(
                    ...
                    train_dataloader,
                    mode="train",
                    prefix_acc=prefix_acc_log.item(),
                )
