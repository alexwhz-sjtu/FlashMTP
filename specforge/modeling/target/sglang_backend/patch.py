...
from sglang.srt.distributed import init_model_parallel_group as _sglang_init_model_parallel_group
...
def init_model_parallel_group(*args, **kwargs):
    """Call the installed SGLang group factory with supported options only."""
    supported = inspect.signature(_sglang_init_model_parallel_group).parameters
    kwargs = {key: value for key, value in kwargs.items() if key in supported}
    return _sglang_init_model_parallel_group(*args, **kwargs)
...
def initialize_dp_attention(
    ...
    """Initialize DP-attention metadata for the embedded SGLang runner."""
