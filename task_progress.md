...
## Attempt 2
我退让一步，我先引入一个单层，一个头的attention模块。把历史的每个位置的融合hs都输入。接着，用最新的CHS和历史融合hs做cross-attention。作用是让最新的chs包含更多的历史信息，有足够的语义信息能够预测未来的一块内容。这里的位置编码就用自然的顺序。chs是anchor-1，往前递减。
对于CHS和融合hs的构造方式，不用现在的attention混合，而是按照dflash的方式，等间隔挑选五层，沿着特征维度拼接，再用一个fc降维。之后草稿模型计算不变