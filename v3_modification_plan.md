...
1. **KL 的方向**（前向 KL $D_{\mathrm{KL}}(P\|Q)$）对固定 $P$、变分 $Q$：在单纯形上，关于 $Q$ 的极小化等价于在期望意义下最小化交叉熵 $\mathbb{E}_{z\sim P}[-\log Q(z)]$。因此 **蒸馏项**是在 Teacher 固定下，对 $q_\phi(\cdot\mid y)$ 做 **加权对数损失** 的一种形式；**单点、固定 Teacher** 时对 $q$ 的凸性可谈，**对神经网络参数**仍整体非凸。

2. **一致性项 + detach**：只对 $q_\phi(\cdot\mid y)$ 反传时，目标是在每个 $i\in S_y$ 上最小化 $D_{\mathrm{KL}}(q_{\phi,\mathrm{sg}}(\cdot\mid y^\*)_i \,\|\, q_\phi(\cdot\mid y)_i)$，即把 **$q(\cdot\mid y)$ 拉向冻结的 $q(\cdot\mid y^\*)$**。这是良定义的凸优化 **在分布空间每个坐标上**；对 $\phi$ 仍是通常的神经网络非凸问题。
